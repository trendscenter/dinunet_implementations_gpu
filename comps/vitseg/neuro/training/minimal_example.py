...
class CustomRunner(Runner):
    ...
    def handle_batch(self, batch):
        ...
        if self.is_train_loader:
            #torch.nn.utils.clip_grad_norm_(self.model.parameters(), .001)
            loss.backward()
            self.optimizer.step()
            scheduler.step()
...
if __name__ == "__main__":
    ...
    logdir = "logs/{model}_same_padding_40_filters_atlas_sv_30".format(model=args.model)
    ...
    ## Set a CosineAealingLRealingLR scheduler
    #T_max = 50
    #eta_min = 1e-10
    #T_mult = 2
